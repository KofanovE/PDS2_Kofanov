...
class AccountTable:
    """
    Клас призначений для створення та видалення csv файлу заданої довжини, у котрому  зберігаються
    логіни зарегістрованих користувачів та їх хешовані паролі.
    Ідея стороннього файлу чесно підглянута у Костянтина Петренка.
    """
...
class Authorization:
    """
    Клас реалізує створення та видалення акаунту та авторизацію в інтерактивному режимі.
    Логіни та захешовані паролі зберігаються у сторонньому csv файлі.
    Пошук та занесення даних до таблиці відбувається з використанням методу подвійного хешування.
    Вирішення колізій відбувається методом відкритої адресації.
    """
    def __init__(self):
